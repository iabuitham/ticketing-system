...
// Handle Delete Table - FIXED: Simplified query without collation issues
if (isset($_GET['delete']) && isset($_GET['id'])) {
    $table_id = intval($_GET['delete']);
    
    // First get the table_number
    $getTableStmt = $conn->prepare("SELECT table_number FROM tables WHERE id = ?");
    $getTableStmt->bind_param("i", $table_id);
    $getTableStmt->execute();
    $tableResult = $getTableStmt->get_result();
    ...
    if ($tableData) {
        ...
        // Simplified check - just look for exact matches
        $check = $conn->prepare("SELECT COUNT(*) as count FROM reservations WHERE table_id = ? AND status NOT IN ('cancelled', 'expired', 'completed')");
        $check->bind_param("s", $table_number);
        $check->execute();
...
// Get all tables - simplified order
...
<head>
    ...
    <style>
        * {
        ...
        body.dark-mode .badge-reserved {
            background: #451a03;
            color: #fde68a;
        }

        body.dark-mode .badge-occupied {
            background: #7f1d1d;
            color: #fca5a5;
        }
...
<div class="container">
    <div class="navbar">
        ...
        <div>
            ...
            <button id="darkModeToggle" class="btn btn-secondary" style="background: #4f46e5;"><i class="bi bi-moon-fill"></i> Dark Mode</button>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($table['section'] ?? '-'); ?></td>
                        <td>
                        ...
                        <td>
                            <button onclick="editTable(<?php echo $table['id']; ?>)" class="btn btn-sm btn-warning" title="Edit Table">
                                <i class="bi bi-pencil"></i>
                            </button>
                            <button onclick="deleteTable(<?php echo $table['id']; ?>)" class="btn btn-sm btn-danger" title="Delete Table">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <select name="section" id="section">
                        ...
                        <option value="Left Circular">Left Circular</option>
                        <option value="Center Left">Center Left</option>
                        <option value="Center Right">Center Right</option>
                        <option value="Right Rectangular">Right Rectangular</option>
                        <option value="Right Circular">Right Circular</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Status</label>
...
<script>
    function openAddModal() {
        ...
        document.getElementById('submitBtn').innerHTML = '<i class="bi bi-plus-lg"></i> Add Table';
        ...
        fetch(`get_table.php?id=${tableId}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('submitBtn').innerHTML = '<i class="bi bi-save"></i> Save Changes';
                    document.getElementById('tableModal').classList.add('active');
                } else {
                    alert('Error: ' + (data.error || 'Could not load table data'));
                }
            })
            .catch(error => {
    ...
    function deleteTable(tableId) {
        if (confirm('⚠️ Are you sure you want to delete this table?\n\nThis cannot be undone.')) {
